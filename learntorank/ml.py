# AUTOGENERATED! DO NOT EDIT! File to edit: ../005_module_ml.ipynb.

# %% auto 0
__all__ = ['Task', 'TextTask', 'SequenceClassification', 'ModelConfig', 'BertModelConfig', 'ModelServer', 'add_ranking_model']

# %% ../005_module_ml.ipynb 4
import sys
...
class ModelServer(ApplicationPackage):
    ...
    def __init__(
        ...
        tasks: Optional[List[Task]] = None,  # List of tasks to be served.
    ):
        super().__init__(
            name=name,
            schema=None,
...
def add_ranking_model(
    ...
    schema=None,  # Name of the schema to add model ranking to.
    include_model_summary_features=False,  # True to include model specific summary features, such as inputs and outputs that are useful for debugging. Default to False as this requires an extra model evaluation when fetching summary features.
    document_field_indexing=None,  # List of indexing attributes for the document fields required by the ranking model.
    **kwargs  # Further arguments to be passed to `RankProfile`.
    ...
    #
    if model_id in app_package.model_ids:
        raise ValueError("model_id must be unique: {}".format(model_id))
    app_package.model_ids.append(model_id)
    app_package.model_configs[model_id] = model_config
    #
    # Export ONNX model
    #
    app_package.get_schema(schema).add_model(model_config.onnx_model())
    #
    # Add query profile type fields
    #
    app_package.query_profile_type.add_fields(*model_config.query_profile_type_fields())
    #
    # Add field for doc token ids
    #
    app_package.get_schema(schema).add_fields(
    ...
    # Add rank profiles
    #
    app_package.get_schema(schema).add_rank_profile(
